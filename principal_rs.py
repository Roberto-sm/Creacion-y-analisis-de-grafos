...
if construir_grafo == "y":
    ...
    while(op!=4):
        ...
        if(op==1):
            ...
            for i in range(0,num_aristas,1):
                ...
                indice_origen = ClaseGrafo.busca_indice(ClaseGrafo.nodos,valor_origen) #indice_origen = guarda el valor de i retornado por la funcion busca_indice, este valor representa la posicion del nodo origen ingresado por el usuario dentro de la lista nodos de la clase grapho 
                valor_destino = input("Teclea el valor del destino ")
                indice_destino = ClaseGrafo.busca_indice(ClaseGrafo.nodos,valor_destino)
            ...
            indice_origen=ClaseGrafo.busca_indice(ClaseGrafo.nodos,valor_origen)
    ...
    while op != 4:
        ...
        if op == 1:
            #en este grafo hicimos la prueba de construccion del mismo mediante parametros
            GrafoCatalogo = catalogo_grafos.grafo1() 
            imprimir_grafo(GrafoCatalogo)

        elif op == 2:   
            #aqui se construye la trayaectoria del grafo2 mediante inputs 
            ...
            if tr == "y":
                ...
                indice_origen = ClaseGrafo.busca_indice(GrafoCatalogo.nodos,trayectoria_origen)
                trayectoria_destino = input("Teclea el valor del node fin de la  trayectoria ")
                indice_destino = ClaseGrafo.busca_indice(GrafoCatalogo.nodos,trayectoria_destino)
                trayectoria = GrafoCatalogo.trayectoria_grado(GrafoCatalogo.nodos[indice_origen],GrafoCatalogo.nodos[indice_destino])
                print(trayectoria)

        elif op == 3:
            #El grafo con el que se trabajó principalmente
